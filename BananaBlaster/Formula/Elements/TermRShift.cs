...
public class TermRShift(Term t1, Term t2) : Term
{
    ...
    private static Term RShift(Term t1, Term t2, int i)
    {
        if (i < 0)
            return t1;
        ...
        return new TermITE(
            ...
            new TermExtraction(
                concat,
                concat.VectorSize - t1.VectorSize, concat.VectorSize - 1
            ),
            RShift(t1, t2, i - 1)
        );
    }
    ...
    public override BoolExpr CreateConstraint(BBContext context)
    {
        ...
        var shiftTerm = RShift(Term1, Term2, Term2.VectorSize - 1);
